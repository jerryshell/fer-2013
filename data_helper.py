...
class DataHelper:
    ...
    def get_batch_value_and_label(self, data_generator: iter, batch_size: int):
        ...
        for batch_index in range(batch_size):
            ...
            batch_value[batch_index] = pixel_list_reshape
        return batch_value, batch_label

    # 训练数据生成器
    def train_data_generator(self, batch_size: int):
        while True:
            yield self.get_batch_value_and_label(self.raw_train_data_generator, batch_size)

    # 验证数据生成器
    def validation_data_generator(self, batch_size: int):
        while True:
            yield self.get_batch_value_and_label(self.raw_validation_data_generator, batch_size)

    # 测试数据生成器
    def test_data_generator(self, batch_size: int):
        while True:
            yield self.get_batch_value_and_label(self.raw_test_data_generator, batch_size)
